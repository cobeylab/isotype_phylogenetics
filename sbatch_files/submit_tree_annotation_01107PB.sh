...
ls ../results/trees/RAxML_bestTree.01107PB_clone_* | grep -o 'clone_[0-9]*'|tr -d [a-z_] > clone_ids_01107PB_annotation.tmp

# Split clone ids in files with 500 ids (so that sbatch commands do not become too long)
split -l 500 clone_ids_01107PB_annotation.tmp 'clone_ids_01107PB_annotation' -a 1
rm clone_ids_01107PB_annotation.tmp
...
do
    ...
    sbatch --array=$CLONE_IDS annotate_trees_01107PB.sbatch
done
