...
ls ../results/trees/RAxML_bestTree.SFAPB_clone_* | grep -o 'clone_[0-9]*'|tr -d [a-z_] > clone_ids_SFAPB_divergence.tmp

# Split clone ids in files with 500 ids (so that sbatch commands do not become too long)
split -l 500 clone_ids_SFAPB_divergence.tmp 'clone_ids_SFAPB_annotation' -a 1
rm clone_ids_SFAPB_divergence.tmp

# Loop over aggregated clone id files, submitting array jobs
for f in clone_ids_SFAPB_annotation*
do
    ...
    sbatch --array=$CLONE_IDS run_divergence_vs_composition_SFAPB.sbatch
done

# Remove temporary files
rm clone_ids_SFAPB_annotation*
