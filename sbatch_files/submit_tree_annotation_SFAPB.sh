...
ls ../results/trees/RAxML_bestTree.SFAPB_clone_* | grep -o 'clone_[0-9]*'|tr -d [a-z_] > clone_ids_SFAPB_annotation.tmp

# Split clone ids in files with 500 ids (so that sbatch commands do not become too long)
split -l 500 clone_ids_SFAPB_annotation.tmp 'clone_ids_SFAPB_annotation' -a 1
rm clone_ids_SFAPB_annotation.tmp
...
do
    ...
    sbatch --array=$CLONE_IDS annotate_trees_SFAPB.sbatch
done
