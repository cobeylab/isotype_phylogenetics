...
ls ../results/trees/RAxML_bestTree.01207PB_clone_* | grep -o 'clone_[0-9]*'|tr -d [a-z_] > clone_ids_01207PB_divergence.tmp

# Split clone ids in files with 500 ids (so that sbatch commands do not become too long)
split -l 500 clone_ids_01207PB_divergence.tmp 'clone_ids_01207PB_annotation' -a 1
rm clone_ids_01207PB_divergence.tmp
...
do
    ...
    sbatch --array=$CLONE_IDS run_divergence_vs_composition_01207PB.sbatch
done
