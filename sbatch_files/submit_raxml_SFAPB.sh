...
ls ../results/clones/SFAPB_clone_*macse* | grep -v 'reduced' | grep -o 'clone_[0-9]*'|tr -d [a-z_] > clone_ids_SFAPB_raxml.tmp


# Split clone ids in files with 500 ids (so that sbatch commands do not become too long)
split -l 500 clone_ids_SFAPB_raxml.tmp 'clone_ids_SFAPB_raxml' -a 1
rm clone_ids_SFAPB_raxml.tmp

# Loop over aggregated clone id files, submitting array jobs
for f in clone_ids_SFAPB_raxml*
do
    ...
    sbatch --array=$CLONE_IDS run_raxml_SFAPB.sbatch
done

# Remove temporary files
rm clone_ids_SFAPB_raxml*
