...
ls ../results/clones/01107PB_clone_*macse* | grep -v 'reduced' | grep -o 'clone_[0-9]*'|tr -d [a-z_] > clone_ids_01107PB_raxml.tmp


# Split clone ids in files with 500 ids (so that sbatch commands do not become too long)
split -l 500 clone_ids_01107PB_raxml.tmp 'clone_ids_01107PB_raxml' -a 1
rm clone_ids_01107PB_raxml.tmp

# Loop over aggregated clone id files, submitting array jobs
for f in clone_ids_01107PB_raxml*
do
    ...
    sbatch --array=$CLONE_IDS run_raxml_01107PB.sbatch
done

# Remove temporary files
rm clone_ids_01107PB_raxml*
