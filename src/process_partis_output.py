    ...
    with open(partis_file_name, 'r') as partis_file:
        ...
        for row in partis_csv:
            if row_counter in largest_clone_indices:
                ...
                seq_ids = [seq_ids[i] for i in range(len(seq_ids)) if in_frame[i] == 'True' and has_stop_codons[i] == 'False']

                clone_sequences = row['input_seqs'].split(':')
                clone_sequences = [clone_sequences[i] for i in range(len(clone_sequences)) if in_frame[i] == 'True' and has_stop_codons[i] == 'False']
